...
namespace CapaLogica
{
    ...
    {
        private readonly ReservaBD reservaBD = new ReservaBD();

        // Método que permite guardar una nueva reserva de videojuegos.
        public bool GuardarReserva(ReservaEntidad reserva, out string mensaje)
        {
            return reservaBD.GuardarReserva(reserva, out mensaje);
        }

        // Método que permite consultar todas las reservas realizadas por un cliente específico.
        public List<ReservaEntidad> ConsultarPorCliente(int idCliente, out string mensaje)
        {
            mensaje = string.Empty;

            try
            {
                return reservaBD.ConsultarPorCliente(idCliente);
            }
            catch (Exception ex)
            {
                mensaje = "Error al consultar reservas: " + ex.Message;
                return new List<ReservaEntidad>();
            }
        }
